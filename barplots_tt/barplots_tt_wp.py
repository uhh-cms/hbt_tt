...
# define different hh output node values
wp_value = [0, 0.5, 0.7, 0.9, 0.95]
working_points = ['none', 'loose', 'medium', 'tight', 'very tight']

for value, working_point in zip(wp_value, working_points):
    tt_sl = []
    tt_dl = []
    tt_fh = []

    for mask in masks:
        tt_wp_mask = events_tt.run3_dnn_moe_hh >= value

        mask = [m & tt_wp_mask for m in mask]
    ...
    ax.set_title(f'subprocesses of the tt background, HH output node $\geq {value}$')
    ax.legend()
    ax.set_yscale('log')

    plt.savefig(f"plots/tt_category_barplot_wp={working_point}.png", dpi=300, bbox_inches='tight')
    plt.show()
    ax.clear()
