...
for key in all_sigs_per_bin.keys():
    ax.errorbar(x_lin_bincenters, all_sigs_per_bin[key]["per_bin"] - all_sigs_per_bin["baseline"]["per_bin"], 
                ...
                label=key+fr"; total $Z_A$: {round(all_sigs_per_bin[key]['total'], 2)}", 
                alpha=1.0, elinewidth=0.5, capsize=2)# , errorevery=2

ax.set_xlabel("DNN output node score")
ax.set_ylabel(r"$Z_A$")
# ax.set_yscale("log")
ax.set_xticks(x_lin_binedges)
ax.set_xticklabels(x_lin_binedges.astype(int), rotation=45)

plt.legend()
plt.title("Difference in Asimov significance for all tested DNN's")
plt.savefig(f"images_all_sigs/all_sigs", dpi=300, bbox_inches='tight')
plt.show()