...
ax1.set_xlabel('HH output node')
ax1.set_ylabel('Number of events', color="black")
ax1.step(x, two_matches2tau.values(), alpha=0.9, label=r"2 matched $\tau$s", color='green')
...
ax1.step(x, two_matches2e.values(), alpha=0.9, label=r"2 matches: 2 $e$'s fake $\tau$'s", color='tab:orange')
...
ax1.step(x, one_match1mu.values(), alpha=0.9, label=r"One matched $\mu$ fakes $\tau$", color='red')
...
ax1.set_ylim(bottom=1e-1)
fig.tight_layout()
plt.title(fr"HH output node; tt background events, tautau channel (which means only dl decay) split in nb of $\tau$ fakes and number of reco $\tau$ matched to gen W children (matching criterion: $\Delta R < ${delr_cut_tau})", wrap=True)
plt.savefig("analysis_tautau/dnn_tau_matching_allevents", dpi=300, bbox_inches='tight')
plt.show()
