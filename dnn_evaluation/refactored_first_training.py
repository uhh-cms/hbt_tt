...
data_dnn_outputs = [
    ...
    process_loader.load_process(path_dnn+"/22_23_lr_linear_a_test_test.pt", label="22_23_lr_linear_a_test", description="Weighting: (1,1,1); cosine-shaped LR"),
...
for d in data_dnn_outputs:
    ...
    events_etau = d.split_into_categories("etau")
    embed() 
    # access all events
    ...
    hists = [HistFab("all_tt_hist", ["tt_dl", "tt_sl", "tt_fh"], "red", "tt: all events", flavor=d.flavor),
                ...
                HistFab("dl_hist", ["tt_dl"], "tab:orange", "tt: dl events", flavor=d.flavor),# or
                HistFab("fh_hist", ["tt_fh"], 'tab:brown', "tt: fh events", flavor=d.flavor),
                HistFab("dy_hist", ["dy"], "tab:pink", "dy: all events", flavor=d.flavor),# '#3B5B92'
    ...
    # TODO: loop through hists instead of this error-prone stuff I'm doung here
    ...
    ax1.stairs(add_flow_bin(histograms["sl_hist"]), edges = x_lin_binedges, linewidth=1.5, baseline=0, fill=False, edgecolor=hists[1].color, label=r"tt: sl decay")
    ax1.stairs(add_flow_bin(histograms["dl_hist"]), edges = x_lin_binedges, linewidth=1.5, baseline=0, fill=False, edgecolor=hists[2].color, label=r"tt: dl decay")
    ax1.stairs(add_flow_bin(histograms["fh_hist"]), edges = x_lin_binedges, linewidth=1.5, baseline=0, fill=False, edgecolor=hists[3].color, label=r"tt: fh decay")
    ax1.stairs(add_flow_bin(histograms["hh_hist"])*scaling_factor, edges = x_lin_binedges, linewidth=1.5, baseline=0, fill=False, edgecolor=hists[5].color, label=fr"signal x {round(scaling_factor)}")
    ax1.stairs(add_flow_bin(histograms["dy_hist"]), edges = x_lin_binedges, linewidth=1.5, baseline=0, linestyle=":", fill=False, edgecolor=hists[4].color, label=r"dy")
    ...
    # fig.tight_layout()
    plt.title(fr"HH output node for signal ($\kappa_\lambda = 1, \kappa_t = 1$) and background; {d.description} test data", wrap=True, pad=13)
