...
data_dnn_outputs = [
    ...
    process_loader.load_process(path_dnn+"/alleras_tt_111_a.pt", label="alleras_largelr", description="Weighting: (1,1,1); cosine-shaped LR"),
...
for d in data_dnn_outputs:
    ...
    for dataset in ["training", "validation", "test"]:
        ...
        hists = [HistFab("all_tt_hist", ["tt_dl", "tt_sl", "tt_fh"], "red", "tt: all events", flavor=d.flavor),
                 ...
                 HistFab("dy_hist", ["dy"], '#3B5B92', "dy: all events", flavor=d.flavor),
        ...
        # fig.tight_layout()
        plt.title(fr"HH output node for signal ($\kappa_\lambda = 1, \kappa_t = 1$) and background; {d.description}; {dataset} data", wrap=True, pad=13)
        plt.savefig(f"images_ref/first_training_{d.label}_{dataset}_ttdy", dpi=300, bbox_inches='tight')
        plt.show()
        plt.close()
