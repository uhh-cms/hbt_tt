...
# split the tt bg data in three processes
    ...
    ax1.step(x, hh_hist.values()*scaling_factor, where="post",  alpha=0.9, label=fr"signal x {round(scaling_factor)}", color="black")
    ax1.step(x, all_tt_hist.values(), where="post", alpha=0.9, label=r"tt: all events", color='red')
    ax1.step(x, sl_hist.values(), where="post", alpha=0.9, label=r"tt: sl decay", color='green')
    ax1.step(x, dl_hist.values(), where="post", alpha=0.9, label=r"tt: dl decay", color='blue')
    ax1.step(x, fh_hist.values(), where="post", alpha=0.9, label=r"tt: fh decay", color='tab:brown')
    ax1.step(x, dy_hist.values(), where="post", alpha=0.9, label=r"dy", color='tab:pink')
    ...
    ax1.set_xticklabels([f"{x:.2f}" for x in bin_edges])
    ax2.legend(lines1 + lines2, labels1 + labels2, loc='upper right', bbox_to_anchor=(1.36, 1))
    # plt.legend(fontsize="small")
    ax1.set_xscale("linear")
    # ax1.set_ylim(bottom=1e-1)
    # fig.tight_layout()
    plt.title(fr"HH output node for signal and tt background; {label1}; flat-s binning; total significance: {round(total_significance, 5)}", wrap=True)
    plt.savefig(f"images/first_training_11{label2}_ttdy", dpi=300, bbox_inches='tight')
    plt.show()
