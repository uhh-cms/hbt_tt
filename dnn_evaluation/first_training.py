...
# split the tt bg data in three processes
    ...
    # fig.tight_layout()
    plt.title(fr"HH output node for signal and tt background; {label1}; flat-s binning; total Asimov significance: $Z_A$ = {round(all_sig_tot[0], 5)}", wrap=True, pad=13)
...
# get bin edges for flat s binning
bin_edges = def_equbin(torch.from_numpy(ak.to_numpy(convert_to_logit(events_hh))), bin_num = n_bins-1, hist_edge_l=lower_border)[2]
bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])
# check if two bin edges are the same
for i in range(len(bin_edges)):
    for j in range(len(bin_edges)):
...
# ax1.set_ylim(bottom=1e-1)
# fig.tight_layout()
plt.title(fr"HH output node for signal and tt background; original input file; flat-s binning; total Asimov significance: $Z_A$ = {round(all_sig_tot[0], 5)}", wrap=True, pad=13)
